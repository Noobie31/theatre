...
export default function NewReportPage() {
  ...
  const handleInputChange = (sectionIndex: number, rowIndex: number, field: string, value: string) => {
    ...
    const gst18 = classVal > 100 ? disGross * (18 / 118) : 0;
    const gst12 = classVal > 100 ? disGross * (12 / 112) : 0;

    section.calculated[rowIndex] = { disGross, gross, gst18, gst12 };
    ...
    for (let i = 0; i < NUM_ROWS; i++) {
      ...
      totals.nett += parseFloat(row.nett) || 0;
              ...
              <tbody>
                {section.rows.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    ...
                    <td className="border p-1">{section.calculated[rowIndex].gross.toFixed(2)}</td>
                    <td className="border p-1">{section.calculated[rowIndex].disGross.toFixed(2)}</td>
                    <td className="border p-1">
                      <input
                        type="number"
                        value={row.nett}
                        onChange={(e) => handleInputChange(sectionIndex, rowIndex, 'nett', e.target.value)}
                        className="w-full p-1 border"
                      />
                    </td>
                    <td className="border p-1">{section.calculated[rowIndex].gst18.toFixed(2)}</td>
                    <td className="border p-1">{section.calculated[rowIndex].gst12.toFixed(2)}</td>
                    <td className="border p-1">
                      <input
                        type="number"
                        value={row.online}
